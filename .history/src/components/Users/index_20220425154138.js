...
//   }
  ...
  function SetBlocked(a,b,c) {
    let my_data = data.map(i=> i.id===b ? i.isBlocked = true: i);

    // my_data[c].isBlocked = (a ='on' ? true: false) 
    // console.log(data[c].isBlocked)
          ...
          {data.map((i,index) => (
            <p className="data">
              ...
              <input
                    ...
                    onChange={(e) => SetVerified(e.target.value,i.id,index)}
                  /></label><label>
                    <input
                    type="checkbox"
                    checked={i.isBlocked}
                    onChange={(e) => SetBlocked(e.target.value,i.id,index)}
                  /></label>             
            </p>
          ))}
        </section>
