  ...
  return (
    <>
      ...
      <div className="main_categories">
        <header>
          <span style={{ width: "15%" }}>wallet</span>
        ...
        <section>
          {data.map((i, index) => (
            <p className="data" key={i.id}>
              <span style={{ width: "15%"}} onClick={()=> WriteToClipboard(i.wallet.address)}>
                {i.wallet.address.slice(0,8)}<strong>...</strong>{i.wallet.address.slice(40,100)}
